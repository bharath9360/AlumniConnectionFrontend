...
export const ChatItem = ({ chat, isActive, onClick }) => {
    ...
    const isMine = lastMsg?.senderId === 'alumni_1';
        ...
        >
            ...
            <div className="flex-grow-1 overflow-hidden">
                <div className="d-flex justify-content-between align-items-center">
                    ...
                    <span className="text-muted" style={{ fontSize: '0.72rem' }}>{lastMsg?.timestamp}</span>
                </div>
...
ChatWindow.Messages = function ChatWindowMessages({ messages }) {
    ...
    return (
        <div className="flex-grow-1 overflow-auto px-4 py-3 d-flex flex-column gap-2">
            {messages.map(msg => (
                <ChatBubble key={msg.id} msg={msg} isMine={msg.senderId === 'alumni_1'} />
            ))}
            <div ref={endRef} />
        </div>
    );
