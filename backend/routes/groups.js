...
router.post('/', protect, authorize('admin'), asyncHandler(async (req, res, next) => {
  ...
  res.status(201).json({
    ...
    message: userIds.length > 0 ? `Group created with ${userIds.length} members auto-added.` : 'Group created (no matching members found).'
  });
}));
...
router.delete('/:groupId', protect, authorize('admin'), asyncHandler(async (req, res, next) => {
...
router.get('/:groupId/members', protect, authorize('admin'), asyncHandler(async (req, res, next) => {
  const members = await GroupMember.find({
    groupId: req.params.groupId
  }).populate('userId', 'name email role department batch profilePic').sort({
    joinedAt: -1
  });
  res.json({
    success: true,
...
router.post('/:groupId/members', protect, authorize('admin'), asyncHandler(async (req, res, next) => {
  const {
    userIds
  } = req.body;
  const {
  ...
  res.json({
    success: true,
    message: `Added ${addedCount} members to the group`
  });
}));

// ─── POST /api/groups/:groupId/bulk-add ────────────────
// Bulk add members by department or batch
router.post('/:groupId/bulk-add', protect, authorize('admin'), asyncHandler(async (req, res, next) => {
  const {
    department,
    batch
  } = req.body;
  ...
  if (!department && !batch) {
    return res.status(400).json({
      message: 'Please provide a department or batch to filter'
    });
  }
  const query = {
    status: 'Active'
  ...
  res.json({
    success: true,
    message: `Added ${addedCount} members to the group out of ${userIds.length} matching users`
  });
}));

// ─── DELETE /api/groups/:groupId/members/:userId ───────
// Remove a member from a group
router.delete('/:groupId/members/:userId', protect, authorize('admin'), asyncHandler(async (req, res, next) => {
  const {
    groupId,
    userId
  } = req.params;
...
router.get('/utils/departments-batches', protect, authorize('admin'), asyncHandler(async (req, res, next) => {
