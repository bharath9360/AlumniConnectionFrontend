...
router.get('/analytics', protect, authorize('admin'), asyncHandler(async (req, res, next) => {
  ...
  const recentRegistrations = await User.find().sort({
    createdAt: -1
  }).limit(7).select('name role department batch status createdAt profilePic');

  // ── Recent Posts (last 5) ───────────────────────────────────
  const recentPosts = await Post.find().sort({
    createdAt: -1
  }).limit(5).populate('userId', 'name profilePic role').select('content media createdAt userId userName userRole');
  ...
  // ── Recent Jobs (last 5) ─────────────────────────────────────
  const recentJobs = await Job.find().sort({
    createdAt: -1
  }).limit(5).populate('postedBy', 'name role').select('title company status createdAt postedBy');
  res.json({
    success: true,
    data: {
      kpis: {
