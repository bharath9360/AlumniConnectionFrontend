...
} = require('../utils/messagingPermissions');
const {
  createNotification
...
router.get('/users/search', protect, asyncHandler(async (req, res, next) => {
  ...
  let acceptedIds = connections.map(c => c.user1.toString() === myId.toString() ? c.user2.toString() : c.user1.toString());
  ...
  const users = await User.find(query).select('name role department presentStatus profilePic email designation company').limit(20);
  res.json(users);
}));
...
router.post('/', protect, requireFields('userId'), asyncHandler(async (req, res, next) => {
  const {
    userId
  } = req.body;
  if (!userId) {
  ...
  try {
    ...
    const hasPermission = await checkMessagingPermission(req.user._id, userId, req.user.role, targetUser.role);
    if (!hasPermission) {
...
router.get('/:chatId/messages', protect, validateObjectId('chatId'), paginationGuard, asyncHandler(async (req, res, next) => {
  const {
    chatId
  } = req.params;
  const myId = req.user._id;
  ...
  const messages = await Message.find({
    ...
    createdAt: -1
  }).skip(skip).limit(limit).populate('senderId', 'name profilePic').populate('chatId');
...
router.post('/:chatId/messages', protect, validateObjectId('chatId'), asyncHandler(async (req, res, next) => {
  const {
    text,
    image
  } = req.body;
  ...
  try {
    ...
    if (chat.isGroupChat) {
      ...
      if (!chat.isMentorshipChat) {
        ...
        if (targetUserId) {
          ...
          if (!hasPerm) {
            return res.status(403).json({
              message: 'You no longer have messaging permission with this user.',
              code: 'NOT_CONNECTED'
            });
          }
        }
    ...
    if (image && text) messageType = 'mixed';else if (image) messageType = 'image';
    ...
    if (chat.isGroupChat) {
      ...
      recipientIds = members.map(m => m.userId.toString()).filter(id => id !== senderId);
    } else {
      recipientIds = chat.participants.map(p => p.toString()).filter(id => id !== senderId);
    }
