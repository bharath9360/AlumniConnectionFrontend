...
router.get('/requests', protect, asyncHandler(async (req, res, next) => {
...
router.get('/sent-requests', protect, asyncHandler(async (req, res, next) => {
...
router.post('/request/:userId', protect, asyncHandler(async (req, res, next) => {
  ...
  if (!myId) return res.status(400).json({
...
router.put('/accept/:requestId', protect, asyncHandler(async (req, res, next) => {
  ...
  if (!reqDoc) {
    return res.status(404).json({
      message: 'Pending connection request not found or unauthorized.'
    });
  }
  ...
  const [senderCount, receiverCount] = await Promise.all([Connection.countDocuments({
    $or: [{
      user1: reqDoc.sender
    }, {
      user2: reqDoc.sender
    ...
    $or: [{
      user1: reqDoc.receiver
    }, {
      user2: reqDoc.receiver
    }]
  })]);
  await Promise.all([User.findByIdAndUpdate(reqDoc.sender, {
    connectionCount: senderCount.toString()
  }), User.findByIdAndUpdate(reqDoc.receiver, {
    connectionCount: receiverCount.toString()
  })]);
...
router.put('/reject/:requestId', protect, asyncHandler(async (req, res, next) => {
  const requestId = req.params.requestId;
  const myId = req.user._id;
  if (!mongoose.Types.ObjectId.isValid(requestId)) {
    return res.status(400).json({
...
router.delete('/remove/:userId', protect, asyncHandler(async (req, res, next) => {
  const targetId = req.params.userId;
  const myId = req.user._id;
  if (!mongoose.Types.ObjectId.isValid(targetId)) {
    return res.status(400).json({
  ...
  // Update connection counts
  const [senderCount, targetCount] = await Promise.all([Connection.countDocuments({
    $or: [{
      user1: myId
    }, {
      user2: myId
  ...
  await Promise.all([User.findByIdAndUpdate(myId, {
    connectionCount: senderCount.toString()
  }), User.findByIdAndUpdate(targetId, {
    connectionCount: targetCount.toString()
  })]);
  res.json({
    success: true,
...
router.get('/status/:userId', protect, asyncHandler(async (req, res, next) => {
  const targetId = req.params.userId;
  const myId = req.user._id;
  if (!mongoose.Types.ObjectId.isValid(targetId)) {
    return res.status(400).json({
...
router.get('/my-connections', protect, asyncHandler(async (req, res, next) => {
  const myId = req.user._id;
  ...
  const otherIds = conns.map(c => c.user1.toString() === myId.toString() ? c.user2 : c.user1);
  const users = await User.find({
    _id: {
      $in: otherIds
    }
  }).select('name role department batch graduationYear profilePic company designation presentStatus connectionCount').lean();
  res.json({
    success: true,
    data: users
  });
...
router.get('/suggestions', protect, asyncHandler(async (req, res, next) => {
  ...
  const [myConns, pendingReqs] = await Promise.all([Connection.find({
    $or: [{
      user1: myId
    }, {
      user2: myId
    }]
  }).lean(), ConnectionRequest.find({
    $or: [{
      sender: myId
    }, {
      receiver: myId
  ...
  const connectedIds = myConns.map(c => c.user1.toString() === myId.toString() ? c.user2.toString() : c.user1.toString());
  const pendingIds = pendingReqs.map(r => r.sender.toString() === myId.toString() ? r.receiver.toString() : r.sender.toString());
  ...
  const candidates = await User.find(baseQuery).select('name role department batch graduationYear profilePic company designation presentStatus connectionCount').limit(poolSize).lean();
